...
namespace GymManager.Helpers
{
    public class PdfHelper : IDisposable
    {
        ...
        public PdfHelper(string caminho)
        {
            ...
            FonteNormal = PdfFontFactory.CreateFont(StandardFonts.HELVETICA);

            FonteNegrito =PdfFontFactory.CreateFont(StandardFonts.HELVETICA_BOLD);
        }
        ...
        public void Campo(string nome, string valor)
        {
            ...
            p.Add(new Text(nome + ": ").SetFont(FonteNegrito));

            p.Add(new Text(string.IsNullOrWhiteSpace(valor) ? "-" : valor).SetFont(FonteNormal));
        ...
        {
            Table tabela = new Table(UnitValue.CreatePercentArray(larguras));
        ...
        public void AdicionarCabecalhoTabela(Table tabela,params string[] colunas)
        {
            foreach (string coluna in colunas)
            {
                Cell cell = new Cell();

                cell.SetBackgroundColor( new DeviceRgb(30, 64, 175));

                cell.SetPadding(8);

                cell.SetBorder(Border.NO_BORDER);

                Paragraph p = new Paragraph(coluna);
        ...
        public void AdicionarLinhaTabela( Table tabela,params string[] valores)
        {
        ...
        public void AdicionarTabela(Table tabela)
        {
            Documento.Add(tabela);
        }
        public void CaixaInformacao( string titulo, string valor)
        {
            ...
            cell.SetBorder( new SolidBorder(new DeviceRgb(220, 220, 220),1));
            ...
            p1.SetFontColor( new DeviceRgb(30, 64, 175));

            cell.Add(p1);

            Paragraph p2 = new Paragraph(valor);
