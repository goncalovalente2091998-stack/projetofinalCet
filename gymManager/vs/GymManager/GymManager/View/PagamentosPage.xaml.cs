...
namespace GymManager.View
{
    public partial class PagamentosPage : Page
    {
            ...
            List<Pagamento> lista)
        {
            ...
            txtTotalPendentes.Text =
                totalPendentes.ToString();

            txtTotalFalhados.Text =
                totalFalhados.ToString();
        }
        ...
        StringComparison.OrdinalIgnoreCase))
            {
                Mensagem.Aviso(
                    "Um pagamento reembolsado não pode ser editado.");

                return;
            }
            PagamentoForm form =
                new PagamentoForm(pagamento)
                {
                    Owner = Window.GetWindow(this)
                };
     ...
     string.Equals(
         ...
         StringComparison.OrdinalIgnoreCase);
            ...
            {
                Mensagem.Aviso(
                    "Este método não permite confirmação manual.");
            ...
            if (!Mensagem.Confirmar(
                $"Confirma a receção do pagamento de " +
                $"{valorFormatado} de '{pagamento.NomeCliente}'?"))
            {
                return;
            }
    ...
    RoutedEventArgs e)
            ...
            TextChangedEventArgs e)
        {
            ...
            {
                Mensagem.Erro(
                    "Não foi possível pesquisar os pagamentos.\n\n" +
                    ex.Message);
            }
        }
        ...
        Pagamento pagamento)
            ...
            if (!Mensagem.Confirmar(
                $"Tem a certeza que pretende reembolsar " +
                $"{valorFormatado} a '{pagamento.NomeCliente}'?"))
            {
                return;
            }
            ...
            try
            {
                ...
                Mensagem.Sucesso(
                    "Pagamento marcado como reembolsado.");
