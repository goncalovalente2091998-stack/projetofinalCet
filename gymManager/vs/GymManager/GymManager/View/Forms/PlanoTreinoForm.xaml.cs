...
namespace GymManager.View.Forms
{
    ...
    {
        private readonly PlanoTreinoService planoService =
...
private bool atualizarPesquisaCliente;
        ...
        {
            string nomePlano =
                txtNomePlano.Text.Trim();

            if (string.IsNullOrWhiteSpace(
                    nomePlano))
            {
                Mensagem.Aviso(
                    "Introduza o nome do plano.");
     ...
     clienteSelecionado.Nome;
            ...
            string nomePT =
                cmbPT.SelectedItem is PersonalTrainer pt
                    ? pt.Nome
                    : string.Empty;
            ...
            {
                Mensagem.Erro(
                    "Não foi possível guardar o plano de treino.\n\n" +
                    ex.Message);
            }
        }
        ...
        private void cmbCliente_Loaded(
     object sender,
     RoutedEventArgs e)
        {
            if (cmbCliente.Template.FindName(
                    "PART_EditableTextBox",
                    cmbCliente) is not TextBox tb)
            {
                return;
            }

            tb.TextChanged -=
                ClientePesquisa_TextChanged;

            tb.TextChanged +=
                ClientePesquisa_TextChanged;
        }
        private void ClientePesquisa_TextChanged(
         object sender,
         TextChangedEventArgs e)
        {
            ...
            if (cmbCliente.SelectedItem is Cliente selecionado &&
                string.Equals(
                    texto,
                    selecionado.DescricaoReserva,
                    StringComparison.OrdinalIgnoreCase))
            {
                return;
            }
