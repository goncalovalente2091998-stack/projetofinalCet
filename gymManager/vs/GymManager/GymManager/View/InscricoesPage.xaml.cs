...
namespace GymManager.View
{
    ...
    {
        private readonly InscricaoService service =
        ...
        private void btnEditar_Click(
            ...
            RoutedEventArgs e)
        {
            if (dgInscricoes.SelectedItem is not
                Inscricao inscricao)
            {
                Mensagem.Aviso(
            ...
            {
                Mensagem.Aviso(
                    "Uma inscrição terminada não deve ser editada. " +
                    "Utilize a opção Renovar.");

                return;
            }

            InscricaoForm form =
                new InscricaoForm(inscricao)
                {
                    Owner = Window.GetWindow(this)
                };
            ...
            RoutedEventArgs e)
        {
            ...
            if (!Mensagem.Confirmar(
                $"Pretende renovar a inscrição de " +
            ...
            RoutedEventArgs e)
        {
            ...
            {
                Mensagem.Aviso(
                    "Não é possível eliminar uma inscrição ativa. " +
                    "Cancele primeiro a inscrição.");
       ...
       Inscricao inscricao)
            ...
            if (!Mensagem.Confirmar(
                $"Pretende gerar um novo pagamento para " +
                $"'{inscricao.NomeCliente}'?"))
            {
                return;
            }
